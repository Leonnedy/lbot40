...
let handler  = async (m, { conn, args, usedPrefix, command }) => {
  ...
  if ((args[0] && args[0] == 'plz') || global.conn.user.jid == conn.user.jid) {
    ...
    conn.on('qr', async qr => {
      let scan = await parent.sendFile(m.chat, await qrcode.toDataURL(qr, { scale: 8 }), 'qrcode.png', 'Escanea este QR para convertirte en un bot temporal\n\n1. Haga clic en el punto en la esquina superior derechaHaga clic en el punto en la esquina superior derecha\n2. Toca WhatsApp Web\n3. Scan QR ini \nQR TIC TOC hijo de puta el codigo se vence en 20 segundos', m)
      setTimeout(() => {
    ...
    conn.connect().then(async ({user}) => {
      parent.reply(m.chat, 'Conectado exitosamente con WhatsApp - mu.\n*NOTE: Es sólo un paseo*\n' + JSON.stringify(user, null, 2), m)
      if (auth) return
      await parent.sendMessage(user.jid, `Puede iniciar sesión sin qr con el siguiente mensaje. para obtener el código completo, envíe *${usedPrefix}getcode* para obtener un código preciso`, MessageType.extendedText)
  ...
  } else throw '¡No se pueden crear bots dentro de bots!\n\nhttps://wa.me/' + global.conn.user.jid.split`@`[0] + '?text=.jadibot'
}
handler.help = ['Leobot']
handler.tags = ['jadibot']
handler.command = /^Leobot$/i
handler.owner = false
handler.mods = false
